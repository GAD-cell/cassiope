...
def getLinks():
    ...
    if response.status_code == 200:
        ...
        if datasets_response.status_code == 200:
            ...
            return(f"Failed to get datasets. Status code: {datasets_response.status_code}")
    else:
...
def telecharger_fichier(lien, chemin_destination):
    ...
    with requests.get(lien, stream=True) as r:
        taille_fichier = int(r.headers.get('content-length', 0))  # Taille du fichier à télécharger
        taille_unitaire = 1024  # Taille unitaire en octets
        bar = tqdm(total=taille_fichier, unit='B', unit_scale=True, desc=os.path.basename(chemin_destination), ncols=100)
